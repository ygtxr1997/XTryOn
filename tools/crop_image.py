...
def crop_arr_according_bbox(img: np.ndarray, bbox: np.ndarray,
                            out_h: int = 1024,
                            out_w: int = 768,
                            is_xyrb: bool = True,
                            ):
    ...
    if long_edge == "height":
        crop_w_int = margin_width_min_int * 2 + 1  # 1 indicates center point
        crop_x_int = cx_int - margin_width_min_int
        crop_h_int = int(float(crop_w_int) * (float(out_h) / float(out_w)))
        crop_y_int = cy_int - int(float(crop_h_int) / 2.)
    else:  # "width"
        crop_h_int = margin_height_min_int * 2 + 1  # 1 indicates center point
        crop_y_int = cy_int - margin_height_min_int
