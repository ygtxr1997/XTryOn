...
def main(opts):
    ...
    if step == 1:
        ...
        cloth_types = [
            ...
            # "sweater_0/processed/",
            # "sweater_1/processed/",
            "DressCode/upper/",  # device:0,1,2,3
            "VITON-HD/train/",  # device:4,5,6,7
        ]
        # cloth_type = cloth_types[((cuda_device + 8) - 1) % 8]
        cloth_type = cloth_types[cuda_device // 4]

        in_root = f"/cfs/yuange/datasets/xss/processed/{cloth_type}"
        out_dir = f"/cfs/yuange/datasets/xss/processed/{cloth_type}"
        ...
        proc = Processor(
            root=in_root,
            out_dir=out_dir,
            extract_keys=["pidinet"],
            is_root_standard=True,
            is_debug=False,
            specific_indices=task,
            specific_rank=cuda_device,
            dataset_person_key="warped_person"
        )
        proc.run()
