...
class MatrixAsPilVisualizer:
    """
    PIL visualizer for matrix data
    """
    ...
    def visualize(self, image_bgr, mask, matrix, bbox_xywh) -> PIL.Image:
        self._check_image(image_bgr)
        self._check_mask_matrix(mask, matrix)
        if self.inplace:
            image_target_bgr = image_bgr
            ...
            image_target_bgr = image_bgr * 0
        image_target_gray = cv2.cvtColor(image_target_bgr, cv2.COLOR_BGR2GRAY)
        x, y, w, h = [int(v) for v in bbox_xywh]
        if w <= 0 or h <= 0:
            return image_bgr
        mask, matrix = self._resize(mask, matrix, w, h)
        ...
        mask_bg = (mask == 0)
        matrix_vis = matrix.astype(np.uint8)
        matrix_vis[mask_bg] = image_target_gray[y: y + h, x: x + w][mask_bg]
        image_target_gray[y: y + h, x: x + w] = (
                image_target_gray[y: y + h, x: x + w] * (1.0 - self.alpha) + matrix_vis * self.alpha
        )

        palette = self._get_palette_from_cv2_colormap(self.val_scale, self.cmap)
    ...
    def _get_palette_from_cv2_colormap(self, scale: float = 255. / 24, cv2_map: int = cv2.COLORMAP_PARULA):
