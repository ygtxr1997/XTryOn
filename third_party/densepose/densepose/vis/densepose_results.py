...
from .base import Boxes, Image, MatrixVisualizer, MatrixAsPilVisualizer
...
class DensePosePilVisualizer(DensePoseResultsVisualizer):
    def __init__(
            self,
            inplace=False,
            cmap=cv2.COLORMAP_PARULA,
        ...
        self.mask_visualizer = MatrixAsPilVisualizer(
            inplace=inplace, cmap=cmap, val_scale=val_scale, alpha=alpha
        )
        self.data_extractor = _extract_i_from_iuvarr
        self.segm_extractor = _extract_i_from_iuvarr
    ...
    def visualize(
        ...
        if densepose_result is None or boxes_xywh is None:
            ...
            image_pil = PIL.Image.fromarray(image_rgb.astype(np.uint8))
            return image_pil
    ...
    def visualize_iuv_arr(self, context, iuv_arr: np.ndarray, bbox_xywh) -> None:
        ...
        mask = np.zeros(matrix.shape, dtype=np.uint8)
        mask[segm > 0] = 1
        image_pil = self.mask_visualizer.visualize(image_bgr, mask, matrix, bbox_xywh)
        context["pil"] = image_pil

    def get_image_bgr_from_context(self, context):
        return context["in_bgr"]
